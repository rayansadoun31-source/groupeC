...
def get_gases_ppm(z_km):
    ...
    for i in range(len(layers) - 1):
        ...
        if alpha != 0:
            P_next = P_bases[-1] * math.pow((T1_K / T0_K), (-g * M) / (R * alpha))
        else:
    ...
    # --- H2O (Vapeur d'eau) ---
    # Calcul de la pression de vapeur saturante (Formule de Magnus)
    e_s = 611.2 * math.exp((17.627 * T_C) / (T_C + 243.04))
    
    if z_km <= 11.0:
        # Décroissance de l'humidité relative moyenne avec l'altitude (77% au sol)
        rh = 0.77 * (1.0 - z_km / 12.0)
        h2o_melange = (rh * e_s / P) * 1000000.0
    else:
        # Plancher stratosphérique constant du au "piège froid" de la tropopause
        h2o_melange = 4.0
        
    # Sécurité pour éviter que la transition troposphère/stratosphère ne descende sous le plancher
    if h2o_melange < 4.0:
        h2o_melange = 4.0
