...
if est_modele_simplifie:
    ...
    # Création explictie des axes géographiques pour l'interface interactive
    ...
    HIRES_STABILIZED_FILE = NPY_DIR / "grid_hires_stabilized_FORCAGE.npy"
    HIRES_ONEYEAR_FILE = NPY_DIR / "grid_hires_1yr_FORCAGE.npy"
    ...
    def upscale_grid(data_lowres, lat_low, lon_low, lat_hi, lon_hi):
        """
        Moteur d'interpolation spatiale (Bilinéaire) : Prend une matrice basse résolution 
    ...
    def f_rhs(T, phinet, C, q_latent, p_cond, forcage_atm):
        ...
        Somme des flux : Solaire net - Évaporation + Forçage Atmosphérique - Émission Infrarouge + Conduction.
        """
        return (
            phinet
            - q_latent
    ...
    def integrate_point_temperature(days, lat_rad, lon_deg, alb_sol_daily, alb_nuages_daily, C_const, q_base, forcage_pixel, T0):
        """
        Résolution temporelle locale : Intègre la température pour un pixel spécifique sur toute 
        la durée de la simulation, incluant le modèle de conduction 1D du sous-sol et 
        le forçage radiatif atmosphérique.
        """
        ...
        for k in range(N_steps):
            ...
            # --- SYNCHRONISATION DU FORÇAGE ---
            jour_index = int(t_sec // 86400)
    ...
    def run_full_hires_simulation(days, stabilize=False):
        ...
        # 1. GÉNÉRATION ET INTERPOLATION DU FORÇAGE ATMOSPHÉRIQUE
        print("Interpolation du forçage atmosphérique sur la grille haute résolution...")
        FORCAGE_ANNUEL = f.preparer_calendrier_forcage(NLAT_HI, NLON_HI, days)
        
        # 2. PHASE DE VECTORISATION ET D'INTERPOLATION DES DONNÉES GÉOSPATIALES
        ...
        # Création des grilles globales de flux de base et d'inertie
        ...
        # Lissage temporel Gaussien annuel
        ...
        # 4. UNIQUE BOUCLE SPATIALE PRINCIPALE DE CALCUL (RÉORGANISÉE)
        for i in tqdm(range(NLAT_HI), desc="Progression (latitude)"):
            for j in range(NLON_HI):
                ...
                # Extraction du vecteur de forçage temporel pour ce pixel spécifique
    ...
    # ────────────────────────────────────────────────
    # LOGIQUE DE CHARGEMENT / SÉLECTION EXECUTION
    # ────────────────────────────────────────────────
    target_file = None
    run_sim = False
    ...
    if run_sim and target_file is None:
        ...
        if choice == '2':
            ...
            # Reste configuré sur 10 jours pour tes tests rapides en local
...
# ────────────────────────────────────────────────
# 5. LE DASHBOARD INTERACTIF MATPLOTLIB
# ────────────────────────────────────────────────
