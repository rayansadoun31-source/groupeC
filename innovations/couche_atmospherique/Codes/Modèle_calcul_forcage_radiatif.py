...
# Assurez-vous que le fichier code_H20_CH4_CO2.py est dans le même dossier
...
# ====================================
# 7. CHARGEMENT DE VOS DONNÉES RÉELLES
# ====================================
print("\nChargement des données réelles...")

# 1. Longueurs d'onde
longueurs_onde = np.loadtxt('longueurs_onde.csv', skiprows=1) 
...
df_T_total = pd.read_csv('temperatures_sol_degre_par_degre.csv')

nb_lignes_1_an = 365 * 180 * 360
if len(df_T_total) >= nb_lignes_1_an:
    df_T_1_an = df_T_total.head(nb_lignes_1_an)
    print("Fichier de températures : 1ère année extraite avec succès.")
else:
    raise ValueError("Le fichier contient moins de données que prévu pour 1 an.")

T_surface_reelle_1D = df_T_1_an['T_sol_K'].values
T_surface_reelle_3D = T_surface_reelle_1D.reshape((365, 180, 360))

# 3. Émissivité du sol (TIFF)
print("Lecture de l'émissivité spatiale (TIFF)...")
emissivite_sfc_3D = tifffile.imread('emissivite_mondiale_365jours.tif')

# --- CORRECTION DE L'AXE TEMPOREL ---
# Si le logiciel SIG a placé les 365 jours à la fin (180, 360, 365)
if emissivite_sfc_3D.shape == (180, 360, 365):
    print("Reformatage de l'émissivité de (Lat, Lon, Jours) vers (Jours, Lat, Lon)...")
    # On déplace l'axe 2 (les jours) en position 0
    emissivite_sfc_3D = np.transpose(emissivite_sfc_3D, (2, 0, 1))

print(f"Dimensions de l'émissivité prêtes pour le calcul : {emissivite_sfc_3D.shape}")

# 4. Albédo des couches atmosphériques (CHARGEMENT DIRECT ET FLUIDE VIA MEMMAP)
print("Chargement direct du fichier d'albédo pré-calculé (Memory-Mapped)...")
chemin_albedo_pret = r"C:\Projet_Climat\albedo_atm_pret_100c_180x360.npy"

# On ouvre le fichier en mode lecture seule ('r') en lui redonnant sa structure exacte
...
print(f"Dimensions de l'albédo chargé : {albedo_atm_4D.shape} (Vérifié et conforme).")

# 5. Émissivité atmosphérique (CONSTANTE TEMPORAIRE)
N_lambda = len(longueurs_onde)
emissivite_atm = np.full((N_couches, N_lambda), 0.05) 
print("⚠️ ATTENTION : Utilisation d'une émissivité atmosphérique constante (0.05) pour les tests.")
...
# =============================================================================
# 10. CALCUL DU BILAN RADIATIF ET DU DLR (FORÇAGE) - TEST 3 JOURS
# =============================================================================
print("\nLancement du calcul global (Test sur 3 jours - Version Vectorisée)...")

N_jours_test = 3

Bilan_Radiatif_Net_3D = np.zeros((N_jours_test, 180, 360))
Forcage_Atmospherique_3D = np.zeros((N_jours_test, 180, 360))

# --- BARRE DE PROGRESSION GLOBALE ---
for jour in tqdm(range(N_jours_test), desc="Calcul Test (Jours)", position=0):
    
    SW_in = calculer_insolation_TOA(latitudes_vecteur, jour)
    SW_descendant = np.repeat(SW_in, 360, axis=1) 
    
    albedo_atm_jour = albedo_atm_4D[jour, :, :, :] 
    ...
    T_sfc_jour = T_surface_reelle_3D[jour, :, :]
    T_atm_jour = T_atm_4D[jour, :, :, :] 
    eps_sfc_jour = emissivite_sfc_3D[jour, :, :]
    
    # --- BARRE DE PROGRESSION SPECTRALE ---
    for w_idx, lam in enumerate(tqdm(longueurs_onde, desc=f"Spectre Jour {jour+1}", position=1, leave=False)):
    ...
    Bilan_Radiatif_Net_3D[jour, :, :] = SW_absorbe_systeme - LW_sortant_jour
    Forcage_Atmospherique_3D[jour, :, :] = LW_descendant_jour

# =============================================================================
# 11. SAUVEGARDE
# =============================================================================
np.save('Bilan_Radiatif_Net_Global_TEST_3_JOURS.npy', Bilan_Radiatif_Net_3D)
np.save('Forcage_Atmospherique_TEST_3_JOURS.npy', Forcage_Atmospherique_3D)

print("\nCalculs terminés. Fichiers de test (3 jours) générés avec succès.")