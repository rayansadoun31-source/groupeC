...
DOSSIER_DONNEES = os.path.join(DOSSIER_RACINE, 'donnees')
...
# =============================================================================
# 3. FONCTIONS DE BASE (PLANCK & SOLEIL) - SÉCURISÉES ANTI-CRASH
# =============================================================================
def planck_radiance(lam, T):
    # Sécurité absolue : Empêche T de descendre sous 0.1 K et évite les divisions par zéro
    T_safe = np.clip(T, 0.1, None)
    lam_safe = np.where(lam <= 0, 1e-99, lam)
    
    # Plafonnage de l'exposant pour éviter l'erreur "overflow in exp"
    exposant = (const.h * const.c) / (lam_safe * const.k * T_safe)
    exposant = np.clip(exposant, None, 700.0)
    
    terme1 = (2 * const.h * const.c**2) / (lam_safe**5)
    terme2 = np.exp(exposant) - 1.0
    radiance = terme1 / terme2
    
    return np.where(lam <= 0, 0.0, radiance)
...
# =============================================================================
# 4. INTÉGRATION HITRAN ET CORRECTION BEER-LAMBERT (Épaisseur des couches)
# =============================================================================
print("\n[1/4] Chargement des données spectrales HITRAN...")

# 1. On FORCE le tri croissant pour éviter le bug des W/m² négatifs !
chemin_ondes = os.path.join(DOSSIER_DONNEES, 'longueurs_onde.csv')
longueurs_onde = np.sort(np.loadtxt(chemin_ondes, skiprows=1)) 
d_lambda = np.gradient(longueurs_onde)

chemin_emi_hitran = os.path.join(DOSSIER_DONNEES, 'table_emissivite_hitran_4_100um_0_100km.csv')
df_emi = pd.read_csv(chemin_emi_hitran)

pivot_emi = df_emi.pivot(index='altitude_km', columns='longueur_onde_um', values='emissivite')
altitudes_hitran = pivot_emi.index.values
ondes_hitran_m = pivot_emi.columns.values * 1e-6
emissivite_hitran_2D = pivot_emi.values

# Interpolation spatiale (fill_value=0.0 empêche les NaNs de faire planter le modèle)
interp_emi = RegularGridInterpolator((altitudes_hitran, ondes_hitran_m), emissivite_hitran_2D, bounds_error=False, fill_value=0.0)
Z_mesh, Lambda_mesh = np.meshgrid(z_layers_km, longueurs_onde, indexing='ij')

# Émissivité brute pour 1000m (Plafonnée à 0.999999 pour éviter log(0))
emissivite_hitran_1km = interp_emi((Z_mesh, Lambda_mesh))
emissivite_hitran_1km = np.clip(emissivite_hitran_1km, 0.0, 0.999999) 

# LOI DE BEER-LAMBERT : On réduit l'émissivité pour l'adapter à nos couches fines (ex: 85m au sol)
tau_hitran_1km = -np.log(1.0 - emissivite_hitran_1km)
facteur_echelle = dz_layers_m[:, np.newaxis] / 1000.0
tau_corrige = tau_hitran_1km * facteur_echelle

emissivite_atm = 1.0 - np.exp(-tau_corrige)
print(f"-> Matrice spectrale calibrée aux vraies épaisseurs : {emissivite_atm.shape}")

# Calcul de l'émissivité Broadband 1D pour le Bilan global
print("-> Calcul des émissivités globales pour le modèle thermodynamique 1D...")
eps_1D = np.zeros(N_couches)
T_ref = 255.0
P_lam_ref = planck_radiance(longueurs_onde, T_ref)
norm = np.sum(P_lam_ref * d_lambda)

for c in range(N_couches):
    eps_1D[c] = np.sum(emissivite_atm[c, :] * P_lam_ref * d_lambda) / norm

# =============================================================================
# 5. LE MOTEUR DU MODÈLE À N COUCHES (SÉPARATION SW / LW)
# =============================================================================
...
# =============================================================================
# 6. RÉSOLUTION SÉCURISÉE (least_squares) - 1D
# =============================================================================
print(f"\n[2/4] Lancement du solveur 1D pour {N_couches} couches...")

T_guess = np.linspace(288.15, 193.15, N_couches + 1)
res = least_squares(bilan_radiatif_N_couches, x0=T_guess, bounds=(100.0, 450.0), loss='soft_l1', xtol=1e-3, ftol=1e-3, max_nfev=500)
T_equilibre = res.x

print(f"-> Surface terrestre moyenne simulée : {T_equilibre[0] - 273.15:>6.2f} °C")

# =============================================================================
# 7. CHARGEMENT DES DONNÉES RÉELLES SPATIALISÉES
# =============================================================================
print("\n[3/4] Chargement des données 3D spatiales (T°, Albédo, Emissivité sol)...")
...
da_T = xr.DataArray(T_brut_3D, coords=[np.arange(N_jours_csv), lats_csv, lons_csv], dims=['jour', 'lat', 'lon'])
...
# Émissivité du sol (TIFF)
chemin_emissivite_sfc = os.path.join(DOSSIER_DONNEES, 'emissivite_mondiale_365jours.tif')
emissivite_sfc_3D = tifffile.imread(chemin_emissivite_sfc)
if emissivite_sfc_3D.shape == (180, 360, 365):
    emissivite_sfc_3D = np.transpose(emissivite_sfc_3D, (2, 0, 1))

# Albédo des couches (MEMMAP)
chemin_albedo_pret = os.path.join(DOSSIER_DONNEES, 'albedo_atm_pret_100c_180x360.npy')
albedo_atm_4D = np.memmap(chemin_albedo_pret, dtype='float32', mode='r', shape=(365, N_couches, 180, 360))
...
# =============================================================================
if res.success:
    delta_T_profil = T_equilibre[1:] - T_equilibre[0]
else:
...
# =============================================================================
print(f"\n[4/4] Calcul global en cours (Jour unique : {DATE_AFFICHEE})...")
...
print(f"- {os.path.basename(nom_fichier_bilan)}")
print(f"- {os.path.basename(nom_fichier_forcage)}")